    ...
    private void startChecker() {
        ...
        appChecker
                ...
                .whenOther(new AppChecker.Listener() {
                    @Override
                    public void onForeground(String packageName) {
                        String Pckn=packageName;
                        if (!packageName.toLowerCase().contains("launcher".toLowerCase())) {
                            if (!(name.equals(packageName))) {
                                i = 0;
                                Toast.makeText(getBaseContext(), "Foreground: " + packageName, Toast.LENGTH_SHORT).show();
                                Intent launchIntent = getPackageManager().getLaunchIntentForPackage("com.rakshith_ACUN.tflite");
                                if (i == 0) {
